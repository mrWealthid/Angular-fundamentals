...
export class EventService {
  ...
  // getSessionBySearch(searchTerm: any): Observable<IEvent[]> {
  ...
  editEvent(eventId: number, newEvents: any) {
    // const findEvent = EVENTS.find((ev) => ev.id === eventId);

    return (EVENTS = EVENTS.map((item) =>
      item.id === eventId ? { ...item, ...newEvents } : item
    ));
  }

  saveSession(event: any) {
    return EVENTS.map((item) =>
      item.id === event.id
        ? { ...item, sessions: [...item.sessions, event] }
        : item
    );
  ...
  // getSessionBySearch(searchTerm:string):Observable<ISession[]>{
  //
  // }
}
